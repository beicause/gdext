...
use godot::obj::{Base, EngineEnum, Gd, InstanceId, NewAlloc, NewGd, WithBaseField};
...
fn create_mesh_arrays() -> Array<Variant> {
    ...
    arrays.set(
        mesh::ArrayType::VERTEX.ord().try_into().unwrap(),
        &vertex.to_variant(),
    );
    arrays
}
...
impl IArrayMesh for InitArrayMeshTest {
    fn init(base: Base<ArrayMesh>) -> Self {
        ...
        sf.base_mut()
            .add_surface_from_arrays(mesh::PrimitiveType::TRIANGLES, &create_mesh_arrays());
        sf
    }

    fn on_notification(&mut self, what: ObjectNotification) {
        if what == ObjectNotification::PREDELETE {
            let arr = create_mesh_arrays();
...
#[itest]
fn base_init_array_mesh() {
    let mesh = InitArrayMeshTest::new_gd();
    assert_eq!(mesh.get_surface_count(), 1);
    assert_eq!(mesh.surface_get_arrays(0), create_mesh_arrays());
}
